...
import { saveTest, getTests, getTestById, updateTest, saveTestApproval } from "../../store/Actions";
...
const TestApprovalForm = (props) => {
  ...
  const [state, setState] = useState({
    ...
    approved:true,
    answers: [],
    approvalUser:user.token.user._id
  })
  ...
  const checkValidity = () => {
    if (state["numberOfQuestions"] === "" || state["instituteName"] === "") {
      ...
      if (editButton) {
        ...
        console.log("saving approvals",state)
        dispatch(
          saveTestApproval(state, (err, response) => {
            if (err) {
              console.log(err)
          ...
          <CCard>
            ...
            <CCardFooter>
              {editButton ?
                ...
                <>
                  {disable ? <></> :
                    <CButton type="submit" size="md" color="primary" onClick={(e) => handleSubmit(e)}><CIcon name="cil-scrubber" /> Add Test</CButton>
                  }
                </>
              }
            </CCardFooter>
